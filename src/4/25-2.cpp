...
double ret_gumbel_pdf(double mu,double sigma,double x)
{  double ret_function,z,tep1;
  
  z=(x-mu)/sigma;
  tep1=exp(-1*z);
  ret_function=tep1*exp(-1*tep1)/sigma;
  return(ret_function); 
  }
double *gumbel_database(double mu,double sigma,int datasize)
{  
     double *datastoreA,*datastoreB,*datastoreC,*data,sum,pdf,x1,y,w;
     double lower,upper,result1,inc;
     int n=100000000,i,j,n1;// n是模擬的次數 
     ...
     datastoreB=(double *)malloc(n*sizeof(double));
     datastoreC=(double *)malloc(n*sizeof(double));
     printf(" 需要一段時間計算 Gumbel(mu=%f,sigma=%f) 分配的資料庫,....\n",mu,sigma);  
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	pdf=ret_gumbel_pdf(mu,sigma,x1);
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
       ...
       alpha1=-10; beta1=10; mu2=0; sigma2=2;
       printf(" 計算 例 25 分配的資料庫,....\n");  
       datastoreA1=uniform_database(alpha1,beta1,n);
       Sortx(0,n,datastoreA1);	//X1的機率模擬器資料庫 
       datastoreA2=gumbel_database(mu2,sigma2,n);

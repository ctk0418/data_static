...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     { 
       ...
       { 
         ...
         tepX1=datastoreA1[j]-0.5; 
         if (tepX1<0) tepX1=-1*tepX1;
         tepX2=datastoreA2[j]-0.5;
         if (tepX2<0) tepX2=-1*tepX2;
         tep=exp(datastoreA1[j]*log(datastoreA2[j]));;
         if (tepX1+tepX2<0.2 || tepX1+tepX2>0.8) { --i; continue;  }
         tepX1=datastoreA1[j];
         tepX2=datastoreA2[j];
         data1[i]=tepX1*cos(tepX2*0.5*M_PI);
         data2[i]=tepX2*cos(tepX1*0.5*M_PI);
      	}  

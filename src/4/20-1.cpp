...
{  double ret_function;
  if (x>=c)  ret_function=lamda*exp(-1*lamda*(x-c)*(x-c));
  else ret_function=0;
  return(ret_function); 
  }
double *rayleigh_database(double lamda,double c,int datasize)
{  
     double *datastoreA,*datastoreB,*datastoreC,*data,sum,pdf,x1,y,w;
     double lower,upper,result1,inc;
     int n=100000000,i,j,n1;// n是模擬的次數 
     ...
     datastoreB=(double *)malloc(n*sizeof(double));
     datastoreC=(double *)malloc(n*sizeof(double));
     printf(" 需要一段時間計算 Rayleigh(lamda=%f,c=%f) 分配的資料庫,....\n",lamda,c);  
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	pdf= ret_rayleigh_pdf(lamda,c,x1);
     ...
     { 
       w=y-inc/2;
       pdf= ret_rayleigh_pdf(lamda,c,w);
       if (pdf<=0) { --i; continue;    }
       datastoreA[i]=w; datastoreB[i]=pdf; 
       sum=sum+pdf;    } 
       n1=i;datastoreC[0]=datastoreB[0]/sum;

...
double *cauchy_database(double mu,double sigma,int datasize)
{  
     ...
     printf(" 需要一段時間計算 Cauchy(mu=%f,sigma=%f) 分配的資料庫,....\n",mu,sigma);  
     j=0;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
       ...
       mu1=0;sigma1=1;mu2=10;sigma2=4;
       printf(" 計算 例 38 分配的資料庫,....\n");  
       datastoreA1=cauchy_database(mu1,sigma1,n);
       Sortx(0,n,datastoreA1);	//X1的機率模擬器資料庫 
       datastoreA2=cauchy_database(mu2,sigma2,n);
       ...
       Joint_plot_control('X',1,'X',2,data1,data2,n,0.02,0.02,0.02,0.02,5);
       printf(" 排序.....\n");
       Sortx(0,n,data1);
       printf(" 完成排序.....\n");
	   Plot_control(data1,n,'X',1,0.02,0.02); 
	   printf(" 排序.....\n");
       Sortx(0,n,data2);
       printf(" 完成排序.....\n");
	   Plot_control(data2,n,'X',2,0.02,0.02); 

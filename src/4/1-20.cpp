...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     { 
       ...
       { 
        ...
        j=serach_index(datastoreC,result1,0,n-1);
         tepX1=datastoreA1[j];
         tepX2=cos(datastoreA1[j]*5*M_PI);
         tep=datastoreA2[j]*cos(datastoreA1[j]*10*M_PI);
         if (tepX1<tepX2|| tepX1>tep) { --i; continue;  }
         if (tepX1<tepX2|| tepX1>tep) { --i; continue;  }
         if (datastoreA1[j]>datastoreA2[j] )
         {
         	data1[i]=datastoreA2[j];   data2[i]=datastoreA1[j];
         }
         else
         {
         	data1[i]=datastoreA1[j];   data2[i]=datastoreA2[j];
         }
      	}  

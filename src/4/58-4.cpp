...
{  
     ...
     { 
       ...
       return(data);   }
double ret_logistic_pdf(double mu,double sigma,double x)
{  double ret_function,z,tep1;
  
  z=(x-mu)/sigma;
  tep1=exp(-1*z);
  ret_function=tep1/((1+tep1)*(1+tep1)*sigma);
  return(ret_function); 
  }
double *logistic_database(double mu,double sigma,int datasize)
{  
     double *datastoreA,*datastoreB,*datastoreC,*data,sum,pdf,x1,y,w;
     double lower,upper,result1,inc;
     int n=100000000,i,j,n1;// n是模擬的次數 
     ...
     datastoreB=(double *)malloc(n*sizeof(double));
     datastoreC=(double *)malloc(n*sizeof(double));
     printf(" 需要一段時間計算 Logistic(mu=%f,sigma=%f) 分配的資料庫,....\n",mu,sigma);  
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	pdf=ret_logistic_pdf(mu,sigma,x1);
     ...
     { 
       w=y-inc/2;
       pdf= ret_logistic_pdf(mu,sigma,w);
       if (pdf<=0) { --i; continue;    }
       datastoreA[i]=w; datastoreB[i]=pdf; 
       sum=sum+pdf;    } 
       n1=i;datastoreC[0]=datastoreB[0]/sum;
...
#include <c:\\simulated_book\\include_dll\DLL_usage_function_name.cpp>
	 ...
	 double *data1,*data2,*data3,sum,pdf,X1,X2,tepX1,tepX2,tep;
	 double mu1,c1,mu2,simga2;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
       ...
       mu1=10;c1=5; mu2=0;simga2=2;
       printf(" 計算 例 58 分配的資料庫,....\n");  
       datastoreA1=arcsin_database(mu1,c1,n);
       Sortx(0,n,datastoreA1);	//X1的機率模擬器資料庫 
       datastoreA2=logistic_database(mu2,simga2,n);

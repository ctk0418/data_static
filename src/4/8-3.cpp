...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     alpha1=-10;beta1=10; lamda2=0.2;c2=-5;
     ...
     {
     	pdf= ret_shifted_exponential_pdf(lamda2,c2,x1);
     	if (pdf>0 && j==0) { j=1; lower2=x1; continue; }
     	if (pdf<=0 && j==1) { j=2; upper2=x1;break; }
     	}
     lower2=c2;
     printf(" 需要一段時間計算 例 8 分配的資料庫,....\n");  
     ...
     { 
       ...
       pdfx2=ret_shifted_exponential_pdf(lamda2,c2,X2);
       pdf=pdfx1*pdfx2;
       if (pdf<=0) { --i; continue;    }

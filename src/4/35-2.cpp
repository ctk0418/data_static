...
double ret_loglogistic_pdf(double alpha,double beta,double x)
{  double ret_function,tep1,tep2;
  if (x>0)
  { 
   tep1=beta/alpha*exp((beta-1)*log(x/alpha));
   tep2=1+exp(beta*log(x/alpha));
   ret_function=tep1/(tep2*tep2);
   if (ret_function<0.0000000001) ret_function=0; }
  else ret_function=0;
  return(ret_function); 
  }
double *loglogistic_database(double alpha,double beta,int datasize)
{  
     double *datastoreA,*datastoreB,*datastoreC,*data,sum,pdf,x1,y,w;
     double lower,upper,result1,inc;
     int n=100000000,i,j,n1;// n是模擬的次數 
     ...
     datastoreB=(double *)malloc(n*sizeof(double));
     datastoreC=(double *)malloc(n*sizeof(double));
     printf(" 需要一段時間計算 Log logistic(alpha=%f,beta=%f) 分配的資料庫,....\n",alpha,beta);  
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	pdf= ret_loglogistic_pdf(alpha,beta,x1);
     ...
     { 
       w=y-inc/2;
       pdf= ret_loglogistic_pdf(alpha,beta,w);
       if (pdf<=0) { --i; continue;    }
       datastoreA[i]=w; datastoreB[i]=pdf; 
       sum=sum+pdf;    } 
       n1=i;datastoreC[0]=datastoreB[0]/sum;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
       ...
       alpha1=-10; beta1=10; alpha2=10; beta2=5; 
       printf(" 計算 例 35 分配的資料庫,....\n");  
       datastoreA1=uniform_database(alpha1,beta1,n);
       Sortx(0,n,datastoreA1);	//X1的機率模擬器資料庫 
       datastoreA2=loglogistic_database(alpha2,beta2,n);
       ...
       Joint_plot_control('X',1,'X',2,data1,data2,n,0,0.02,0.02,0,5);
       printf(" 排序.....\n");
       Sortx(0,n,data1);
       printf(" 完成排序.....\n");
	   Plot_control(data1,n,'X',1,0,0.02); 
	   printf(" 排序.....\n");
       Sortx(0,n,data2);
       printf(" 完成排序.....\n");
	   Plot_control(data2,n,'X',2,0.02,0); 

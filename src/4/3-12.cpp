...
#include <c:\\simulated_book\\include_dll\DLL_usage_function_name.cpp>
	 ...
	 double *data1,*data2,sum,pdf,X1,X2,tepX1,tepX2,count1=0;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     { 
       ...
       { 
         ...
         if (tepX1*tepX2<-10 || tepX1*tepX2>10)  { --i; continue;}
		 data1[i]=(tepX1+tepX2)/(tepX1*tepX2);
		 data2[i]=(tepX1-tepX2)/(tepX1*tepX2);
	    }
	   printf("P(符合要求)=%f\n",(double)n/count1);

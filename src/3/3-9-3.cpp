...
double ret_joint_function3_check(double x1,double x2)
{ 
  double ret_function;
...
int main() {  
     ...
     double lower1,upper1,X1,x1,lower2,upper2,X2,x2,sumfunction,sum,area,MIN,MAX;
	 // upper琌跑计   lower琌跑计 
     ...
     {
     	ret_function=ret_joint_function3_check(x1,x1);
     ...
     { 
       ...
       if (MIN<-10||MAX>12){ --i; continue;  } // 戈ぃ才 

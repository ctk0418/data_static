...
double ret_joint_function4_check(double x1)
{ 
  double ret_function;
  
  if (x1>0)
  {
  	ret_function=exp(-1*x1);
  	if (ret_function<0.0000001) ret_function=0;
  }
  else ret_function=0;
  return(ret_function); 
  }
...
int main() {  
	 ...
	 // upper琌跑计   lower琌跑计 
     int n=100000000,i,j;// n is だ澄计 
     char *ss;
     
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	ret_function=ret_joint_function4_check(x1);
     ...
     {
     	ret_function=ret_joint_function4_check(x2);

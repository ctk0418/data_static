...
double ret_Frechet_pdf(double mu,double sigma,double alpha,double x)
{  double ret_function,z,tep1,tep2,tep3;
  if (x>=0) 
  { 
    z=(x-mu)/sigma;
    tep1=exp(-(alpha+1)*log(z))*alpha/sigma;
    tep2=exp(-1*alpha*log(z));
    tep3=exp(-1*tep2);
    ret_function=tep1*tep3; 
	if (ret_function<0.00000000001) ret_function=0; }
  else ret_function=0;
  return(ret_function); 
  }
...
#include <c:\\simulated_book\\include_dll\DLL_usage_function_name.cpp>
     ...
     double lower,upper,result1,inc,mu,sigma,alpha;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     printf(" 需要一段時間計算 Frechet 分配的資料庫,....\n");  
     mu=2;  sigma=3; alpha=4;
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	pdf=ret_Frechet_pdf(mu,sigma,alpha,x1);
     ...
     { 
       w=y-inc/2;
       pdf= ret_Frechet_pdf(mu,sigma,alpha,w);
       if (pdf<=0) { --i; continue;    }
       ...
       Sortx(0,n,data);
       printf(" 完成排序.....\n");
	   Plot_control(data,n,'X',1,0,0.01); //模擬資料次數分配圖示與統計係數 
	   free(data);

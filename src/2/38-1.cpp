...
double ret_GND_pdf(double mu,double alpha,double beta,double x)
{  double ret_function,z,tep1,tep2;
  
  z=(x-mu);
  if (z<0) z=-1*z;
  if (z>0) tep1=exp(beta*log(z)); else tep1=0;
  tep1=tep1/alpha;
...
#include <c:\\simulated_book\\include_dll\DLL_usage_function_name.cpp>
     ...
     double lower,upper,result1,inc,mu,alpha,beta;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     printf(" 需要一段時間計算 GND 分配的資料庫,....\n");  
     mu=10; alpha=4; beta=2;
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	pdf=ret_GND_pdf(mu,alpha,beta,x1);
     ...
     { 
       w=y-inc/2;
       pdf= ret_GND_pdf(mu,alpha,beta,w);
       if (pdf<=0) { --i; continue;    }
       ...
       Sortx(0,n,data);
       printf(" 完成排序.....\n");
	   Plotx(data,n,'X',1); //模擬資料次數分配圖示與統計係數 
	   free(data);

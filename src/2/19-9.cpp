...
#include <c:\\simulated_book\\include_dll\DLL_usage_function_name.cpp>
     ...
     double *datastoreA,*datastoreB,*datastoreC,*data,sum,pdf,x1,y,w,tep;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     { 
       ...
       { 
         ...
         if ( datastoreA[j]>0)  tep=datastoreA[j];
         else tep=-1*datastoreA[j];
		 if (tep>0) data[i]=atan(datastoreA[j])*exp(0.5*log(tep)); 
		 else data[i]=0;
      	}   
       printf(" 排序.....\n");
       Sortx(0,n,data);

...
double ret_GEV_pdf(double mu,double sigma,double c,double x)
{  double ret_function,z,tep1,tep2,tep3;
  ...
  if (z>-1/c)
  {
  tep1=exp((-1/c-1)*log(1+c*z))/sigma;
  tep2=exp(-1/c*log(1+c*z));
  tep3=exp(-1*tep2);
...
#include <c:\\simulated_book\\include_dll\DLL_usage_function_name.cpp>
     ...
     double lower,upper,result1,inc,mu,sigma,c;
...
#include <c:\\simulated_book\\include_dll\\DLL_function_load.cpp>
     ...
     printf(" 需要一段時間計算 GEV 分配的資料庫,....\n");  
     mu=2; sigma=3;c=3;
     j=0;
     for(x1=-1000000;x1<1000000;x1=x1+1)
     {
     	pdf=ret_GEV_pdf(mu,sigma,c,x1);
     ...
     { 
       ...
       Sortx(0,n,data);
       printf(" 完成排序.....\n");
	   Plot_control(data,n,'X',1,0,0.05); //模擬資料次數分配圖示與統計係數 
	   free(data);
